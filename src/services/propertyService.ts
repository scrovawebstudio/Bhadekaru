...
export const propertyService = {
  async getProperties(): Promise<Property[]> {
    ...
    return state.properties
      ...
      .map((p) => {
        const units = state.units.filter((u) => u.property_id === p.id && u.organization_id === orgId);
        return {
          ...p,
          units,
        };
  ...
  async getPropertyById(id: string): Promise<Property | null> {
    const state = dbStore.getState();
    const orgId = state.organization.id;
    const prop = state.properties.find((p) => p.id === id && p.organization_id === orgId);
    if (!prop) return null;
    const units = state.units
      .filter((u) => u.property_id === id && u.organization_id === orgId)
      .map((u) => {
        const agreement = state.agreements.find((a) => a.unit_id === u.id && a.is_active && a.organization_id === orgId);
        const tenant = agreement ? state.tenants.find((t) => t.id === agreement.tenant_id && t.organization_id === orgId) : undefined;
        return {
          ...u,
  ...
  async getUnits(): Promise<PropertyUnit[]> {
    ...
    return state.units
      ...
      .map((u) => {
        const agreement = state.agreements.find((a) => a.unit_id === u.id && a.is_active && a.organization_id === orgId);
        const tenant = agreement ? state.tenants.find((t) => t.id === agreement.tenant_id && t.organization_id === orgId) : undefined;
        const property = state.properties.find((p) => p.id === u.property_id && p.organization_id === orgId);
        return {
          ...u,
